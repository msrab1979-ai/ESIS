...
**📋 Corak Gred** (`hcMuridCorak`, `hcmKiraCorak`, `hcmCorakLabel`, `hcmBandingCorak`, `hcmTogolCorak`, `hcmCorakCSV`): kad di bawah KPI yang mengumpul murid ikut **kombinasi gred** subjek diutamakan pada ujian terkini — `4A`, `3A1B`, `2A1B1C` (bilangan ikut jumlah subjek utama darjah itu, jadi 6 subjek → `6A`). Setiap baris = satu corak dengan bar + bilangan murid; klik untuk buka senarai **nama + kelas + gred setiap subjek** (accordion, state dalam `hcmCorakBuka`, kekal semasa re-render). Susunan corak dari terbaik ke terlemah — banding bilangan A dahulu, kemudian B, C… (bukan ikut bilangan murid). Murid dengan subjek utama **belum bermarkah diasingkan** ke baris "⚠ Tidak lengkap" — corak `3A1B` tak sah kalau satu gred belum diketahui (konsisten dengan peraturan Full A). Ikut tapisan kelas + kumpulan yang aktif. CSV sendiri (`hcmCorakCSV`) + disertakan dalam `hcMuridPDF`. Bergantung pada medan `gredSubjek` yang disimpan dalam rekod murid oleh `hcLukisMurid`.
